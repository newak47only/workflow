	...
	<div class="wap-container">
		<div class="panel">
			<div class="panel-body">
				<form action="/recode/cirstore/" method="post" class="form form-horizontal" id="form-admin-add">
					...
					<input type="hidden" name="nego_id" value="{{$nego_id}}">
					{{csrf_field()}}
					{{method_field('POST')}}
					<div class="row clearfix">
				...
				submitHandler:function(form){
					$(form).ajaxSubmit({
						success:function(data){
							if( data == '1'){
								layer.msg('工作记录添加成功！',{ icon: 1,time:2000},function(){
								...
								layer.msg('工作记录添加失败！',{ icon: 2,time:2000});
							}
						},
						error:function(XmlHttpRequest,textStatus,errorThrown){
							layer.msg('工作记录添加错误！',{ icon: 1,time:1000});
						}
					});
				},
			});
